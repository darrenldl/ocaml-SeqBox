...
module Header : sig
  ...
  let to_string ~(alt_seq_num:uint32 option) ~(header:t) ~(data:string) : string =
    ...
    | Some seq_num ->
       ...
       *                                    ; seq_num_string
       *                                    ] in *)
      let str_buf = Buffer.create sbx_header_size in
      Buffer.add_string str_buf (header_to_signature header);
      Buffer.add_string str_buf (ver_to_string (header_to_ver header));
      Buffer.add_string str_buf (crc_result);
      Buffer.add_string str_buf (header_to_file_uid header);
      Buffer.add_string str_buf (seq_num_string);
      (* List.iter (fun s -> Buffer.add_string str_buf s) header_parts; *)
      Bytes.to_string (Buffer.to_bytes str_buf)
    | None ->
...
module Block : sig
  ...
  let to_string ?(alt_seq_num:uint32 option) (block:t) : string =
    ...
    let str_buf = Buffer.create (ver_to_block_size (Header.header_to_ver header)) in
    Buffer.add_string str_buf header_string;
    Buffer.add_string str_buf data;
    Bytes.to_string (Buffer.to_bytes str_buf)
  ;;
